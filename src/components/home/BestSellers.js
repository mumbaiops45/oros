...
export default function BestSellers() {
  // The API has no "bestseller" flag and no order history to derive one from,
  // so this rail shows the next five products after the featured grid. Add a
  // flag (or an orders collection) on the backend and this becomes a real
  // ranking without touching the markup.
  ...
  // The five that follow the featured grid. A catalogue smaller than that
  // shows its last five rather than an empty rail.
  const cards = live.map(toCardProduct);
  const products = cards.length > 8 ? cards.slice(8) : cards.slice(-5);
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8 lg:px-15">
        <div className="mb-10 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full bg-white/20 px-4 py-1.5 text-[11px] font-semibold tracking-[0.18em] text-white uppercase ring-1 ring-white/30">
        ...
        <ul className="grid grid-cols-2 gap-4 sm:gap-5 lg:grid-cols-5">
          {loading
            ? [0, 1, 2, 3, 4].map((index) => (
                <li
                  key={index}
                  className="h-80 animate-pulse rounded-2xl border border-white/20 bg-white/10"
                />
              ))
        ...
        {!loading && products.length === 0 && (
          <p className="rounded-2xl border border-dashed border-white/35 px-6 py-14 text-center text-sm text-white/80">
